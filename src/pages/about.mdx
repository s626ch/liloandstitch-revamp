---
layout: "../layouts/MdPage.astro"
title: liloandstit.ch
description: "aloha, i'm Stitch, welcome to my site :D"
---

    <a href="/2021-10-29.jpg" class="rrr ub" target="_blank">
        <img src="/mee.png" alt="literally me!! wearing my stitch costume." />
        Me wearing my Stitch suit, click to expand.
    </a>
    <h3 class="ub wht nfw jst">
        If you really want a complete laundry list of stuff I have done, and can do...
        <ul>
            <li>Web Design</li>
            <li>Web     <h1 class="bb cnt nfw brd">Okie-taka, but what do you really do?</h1>
Development</li>
            <li>Software Development</li>
            <li>System Administration</li>
            <li>Network Infrastructure</li>
            <li>Graphic Design</li>
            <li>Music Production</li>
            <li>Video Editing</li>
            <li>Photo Editing</li>
            <li>3D Rigging</li>
            <li>3D Modeling</li>
            <li>...and probably more I'm struggling to remember.</li>
        </ul>
        This is all stuff I've picked up just doing things as a hobby over the years.<br />
    </h3>