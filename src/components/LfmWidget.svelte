<script lang="ts">
    ...
    async function fetchLatest() {
        let res = await fetch('https://lastfm-api.dev.madhouselabs.net/getRecentTracks?username=s626ch');
...
<style>
    /*lastfm shit*/
    .lastfm {display:flex;align-items:center;justify-content:center;margin-bottom:32px;}
    .box {display: flex;max-width: 420px;padding: 16px;border-left: 4px solid var(--header);background: var(--darkCol);}
    ...
    .artist,.album{font-style:italic;color:var(--pre);}
...
<div class="lastfm">
    <div class="box">
        <a href={track?.url} target="_blank" class="link">
            <img src={track?.image || "/missing.png"} class="art" alt={altTxt} title={altTxt}/>
        </a>
        <div class="info">
            <h2 class="track ub wht ellipse">{track?.name || "..."}</h2>
            <p class="artist ub ellipse">{track?.artist || "..."}</p>
